...
void registerSpace::initialize64() {
    ...
    // TODO: This initialization doesn't consider all registers. Right now only the regs we typically use for instrumentation are considers. This might need work later on.
    ...
    //VGPRs
    for (unsigned idx = vgpr0; idx <= vgpr255; idx++) {
        char name[32];
        sprintf(name, "vgpr%u", idx - vgpr0);
        registers.push_back(new registerSlot(idx,
                                             name,
                                             /*offLimits =*/false,
                                             registerSlot::liveAlways,
