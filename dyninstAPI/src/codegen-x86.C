...
static void generateAMDGPUBranch(codeGen &gen, Address fromAddr,
    ...
    fprintf(stderr, "fromAddr : %lx,    toAddr : %lx \n", fromAddr, toAddr);
    ...
    //
    long long int diff = toAddr;
    diff = diff - fromAddr - 4;
    fprintf(stderr," diff = %llx\n", diff);
    append_memory_as(insn,0xbe800000 | (2<<16) | (28 << 8) );
    append_memory_as(insn,0x80000000 | (0) << 23 | (2) << 16 | (2) << 8 | 0xff);
    append_memory_as(insn,uint32_t{diff&0xffffffff });

    append_memory_as(insn,0x80000000 | (4) << 23 | (3) << 16 | (3) << 8 | 0xff);
    append_memory_as(insn,uint32_t{diff>>32});


    append_memory_as(insn,0xbe801d02);
    SET_PTR(insn, gen);
